...
void TextEditor::SetLanguageDefinition(const LanguageDefinition& aLanguageDef)
{
    ...
    for( LanguageDefinition::TokenRegexString& r : mLanguageDefinition.mTokenRegexStrings )
...
void TextEditor::ToggleBreakpoint(int line)
{
    if( mBreakpoints.count(line) )
    {
        mBreakpoints.erase(line);
    }
    else
    {
...
TextEditor::Line& TextEditor::InsertLine(int aIndex)
{
    ...
    Line& result = *mLines.insert( mLines.begin() + aIndex, Line() );
...
ImU32 TextEditor::GetGlyphColor(const Glyph& aGlyph) const
{
    ...
    ImU32 const color = mPalette[(int)aGlyph.mColorIndex];
    if( aGlyph.mPreprocessor )
    {
        const ImU32 ppcolor = mPalette[(int)PaletteIndex::Preprocessor];
...
void TextEditor::HandleKeyboardInputs()
{
    ...
    bool special_none   = !ctrl && !shift && !alt;
    bool special_not_ca = !ctrl && !alt;
    bool special_c      =  ctrl && !shift && !alt;
    bool special_s      = !ctrl &&  shift && !alt;
    bool special_a      = !ctrl && !shift &&  alt;
    bool special_cs     =  ctrl &&  shift && !alt;
    bool special_ca     =  ctrl && !shift &&  alt;
    bool special_sa     = !ctrl &&  shift &&  alt;
    bool special_csa    =  ctrl &&  shift &&  alt;
    ...
    if( ImGui::IsWindowFocused() )
    {
        ...
        if(      !IsReadOnly() && special_c      && ImGui::IsKeyPressed( ImGuiKey_Z ) )
            Undo();
        else if( !IsReadOnly() && special_a      && ImGui::IsKeyPressed( ImGuiKey_Backspace ) )
            Undo();
        else if( !IsReadOnly() && special_c      && ImGui::IsKeyPressed( ImGuiKey_Y ) )
            Redo();
        else if(                  special_not_ca && ImGui::IsKeyPressed( ImGuiKey_UpArrow ) )
            MoveUp( 1, shift );
        else if(                  special_not_ca && ImGui::IsKeyPressed( ImGuiKey_DownArrow ) )
            MoveDown( 1, shift );
            ...
            MoveBottom( shift );
        else if(                  special_not_ca && ImGui::IsKeyPressed( ImGuiKey_Home ) )
            MoveHome( shift );
        else if(                  special_not_ca && ImGui::IsKeyPressed( ImGuiKey_End ) )
            MoveEnd( shift );
        else if( !IsReadOnly() && special_none   && ImGui::IsKeyPressed( ImGuiKey_Delete ) )
            Delete();
        else if( !IsReadOnly() && special_none   && ImGui::IsKeyPressed( ImGuiKey_Backspace ) )
            Backspace();
        else if(                  special_none   && ImGui::IsKeyPressed( ImGuiKey_Insert ) )
            mOverwrite ^= true;
        else if(                  special_c      && ImGui::IsKeyPressed( ImGuiKey_Insert ) )
            Copy();
        else if(                  special_c      && ImGui::IsKeyPressed( ImGuiKey_C ) )
            Copy();
        else if( !IsReadOnly() && special_s      && ImGui::IsKeyPressed( ImGuiKey_Insert ) )
            Paste();
        else if( !IsReadOnly() && special_c      && ImGui::IsKeyPressed( ImGuiKey_V ) )
            Paste();
        else if(                  special_c      && ImGui::IsKeyPressed( ImGuiKey_X ) )
            Cut();
        else if(                  special_s      && ImGui::IsKeyPressed( ImGuiKey_Delete ) )
            Cut();
        else if(                  special_c      && ImGui::IsKeyPressed( ImGuiKey_A ) )
            SelectAll();
        else if( !IsReadOnly() && special_none   && ImGui::IsKeyPressed( ImGuiKey_Enter ) )
            EnterCharacter( '\n', false );
        else if( !IsReadOnly() && special_not_ca && ImGui::IsKeyPressed( ImGuiKey_Tab ) )
    ...
    if( !mLines.empty() )
    {
        ...
        while( lineNo <= lineMax )
        {
            ...
            if( mBreakpoints.count( lineNo + 1 ) != 0 )
            {
                ...
                ImVec2 end = ImVec2( lineStartScreenPos.x + 10.0f, lineStartScreenPos.y + mCharAdvance.y );
                drawList->AddRectFilled( start, end, mPalette[(int)PaletteIndex::Breakpoint] );
            }

            // Draw current line in debugger.
            {
                ImVec2 start = ImVec2( lineStartScreenPos.x + scrollX, lineStartScreenPos.y );

                if( lineNo+1 == mDebuggerLine )
                {
                    ImVec2 end = ImVec2(lineStartScreenPos.x + contentSize.x + 2.0f * scrollX, lineStartScreenPos.y + mCharAdvance.y);
                    drawList->AddRectFilled( start, end, mPalette[(int)PaletteIndex::DebuggerLine] );
                }
            }
            ...
            if( mState.mCursorPosition.mLine == lineNo )
            {
                ...
                if( focused )
                {
                    ...
                    uint64_t elapsed = timeEnd - mStartTime;
                    if( elapsed > 400 )
                    {
                        float width = 1.0f;
            ...
            ImU32 prevColor = line.empty() ? mPalette[(int)PaletteIndex::Default] : GetGlyphColor( line[0] );
            ImVec2 bufferOffset;
    ...
    case TextEditor::SelectionMode::Line:
        {
            ...
            const size_t lineSize = (size_t)lineNo < mLines.size() ? mLines[lineNo].size() : 0;
...
const TextEditor::Palette& TextEditor::GetDarkPalette()
{
    const static Palette p = { {
            ...
            0xff3050ff, // Breakpoint
            0x3030d030, // DebuggerLine
